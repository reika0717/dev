...
script_sticky.setAttribute('src', '/jekyll-test/script/stickyfill.min.js')
...
var initialize = {
  'index': function() {
    ...
    tags_key.map(function(data) {
      $('a[tag="' + data + '"]').before('<img src="/jekyll-test/img/icon_tag_' + data + '.svg" class="news__tag-icon" alt="" >')
    })
    //タグ名を日本語に変換
      ...
      success: function(data) {
        ...
        for (var i = 0; i < events_array.length; i++) {
          ...
          elements += '<article class="article__section event__section-ja">' +
            '<img src="/jekyll-test/img/event_assets/' + events_array[i][event_img_order] + '">' +
